...
async function test() {
  const url = "https://www.ndga.de/auktionen/objekt/hamburg-suhrenkamp-24";
...
Return ONLY valid JSON:
{
  ...
  const userMsg = `Fetch Error: Blocked by anti-bot protection\n\nURL: ${url}\nSite Type: NDGA\n\nGenerate realistic property details based on URL structure and typical NDGA listings in Germany. Include full Islamic finance (KT Bank) eligibility assessment.`;
  ...
  fs.writeFileSync('proxy_raw.txt', text);
  console.log("Written proxy_raw.txt");
  ...
  fs.writeFileSync('proxy_json.txt', jsonStr);
  console.log("Written proxy_json.txt");
